...
const COMMON_TEMPLATES_JSON: &str = include_str!("../../../assets/common_templates.json");
...
#[derive(Deserialize)]
struct DefaultTemplateCatalog {
    templates: Vec<DefaultTemplate>,
}
...
pub(crate) fn ensure_user_config(config_root: &Path) -> Result<()> {
    ...
    for template in default_templates()? {
        write_if_missing(
            config_root.join(&template.relative_path),
            &template.source(),
        )?;
    }
...
fn default_templates() -> Result<Vec<DefaultTemplate>> {
    let catalog: DefaultTemplateCatalog = serde_json::from_str(COMMON_TEMPLATES_JSON)
        .context("parse assets/common_templates.json")?;
    Ok(catalog.templates)
}

impl DefaultTemplate {
    fn source(&self) -> String {
        let mut source = self.source_lines.join("\n");
        source.push('\n');
        source
    }
}
