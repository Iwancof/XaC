use xac_core::{
    BehaviorKind, DeliveryJob, DroneState, ItemKind, Pos, WorldPos, CARRIER_DRONE_BATTERY_CAPACITY,
    CARRIER_DRONE_BATTERY_RECHARGE_PER_TICK, CARRIER_DRONE_DOCKING_DISTANCE,
    CARRIER_DRONE_LOCAL_CPU_RATE, CARRIER_DRONE_LOGIC_FUEL_CAPACITY,
    CARRIER_DRONE_LOGIC_RECHARGE_PER_TICK, CARRIER_DRONE_MOVE_BATTERY_COST,
    CARRIER_DRONE_MOVE_SPEED, CARRIER_DRONE_WORK_BATTERY_COST,
};
...
impl Simulation {
    ...
    pub(crate) fn charge_docked_drone(&mut self, drone_id: &str) {
        if let Some(drone) = self.drones.get_mut(drone_id) {
    ...
    pub(crate) fn return_drone_to_home(&mut self, drone_id: &str) {
    ...
    pub(crate) fn drone_at_home(&self, drone_id: &str) -> bool {
        let Some(home_pos) = self.drone_home_pos(drone_id) else {
            return false;
        };
        self.drones
